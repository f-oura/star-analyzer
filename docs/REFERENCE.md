...
- **Unified phi femto QA:** `./script/singularity_checkHistAnaFemtoPhi.sh <root_file> <mainconf_path>` — single QA PDF only (representative cent slices × all channel bases; no `*_CF_*.pdf`). Mixing runs add the channel-indexed `hMixSamplerQA` counter page. Production mainconfs keep `randomSample` and receive its corrected eligible-pair semantics. If the unified histogram YAML is regenerated, use `python3 script/generate_hist_anaFemtoPhi.py`; the generator retains the mixing QA axes and histogram.
