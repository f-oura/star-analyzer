...
- **New track species** `phikaon_plus` / `phikaon_minus` (`particleKey: phi_kaon_plus|phi_kaon_minus`). They reuse the exact φ-daughter kaon selection (the in-event `kaonsPlus`/`kaonsMinus` vectors), NOT the anaFemtoKaon `kaon_minus` bachelor cuts. Registered via `speciesKeys` in the maker YAML.
